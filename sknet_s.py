...
class SKLayer(nn.Module):
    # feature输入通道数，m分支数量，g分组卷积，r通道压缩率，stride步幅，L压缩后的通道数最小不能低于
    def __init__(self,features,M=2,G=32,r=16,stride=1,L=32):
        ...
        for i in range(M):
            self.convs.append(nn.Sequential(nn.Conv2d(features,features,kernel_size=3,
                                                      stride=stride,padding=1+i,dilation=1+i,groups=G,bias=False),
        ...
        for i in range(M):
            self.fcs.append(nn.Sequential(nn.Conv2d(d,features,kernel_size=1,stride=1),
                              nn.BatchNorm2d(features),
                              nn.ReLU(inplace=False)))
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0,reduction=16):
        ...
        self.se=SKLayer(out_planes,reduction)
    # 前向传播函数，张量残差处理，主流程
    def forward(self, x):
        # residual = x
